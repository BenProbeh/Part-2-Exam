...
function Sidebar() {
  return (
    <aside className="storm-sidebar fixed left-0 top-0 z-20 flex h-screen w-64 flex-col text-white">
      ...
      <h2 className="sidebar-exam-title px-6 pb-4 text-center text-sm font-bold leading-snug tracking-wide">
        Ben Probeh&apos;s Exam
      </h2>
      ...
      <nav className="flex flex-1 flex-col gap-2 px-4">
        {navItems.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            to={to}
            className={({ isActive }) =>
              `glow-nav flex items-center gap-3 rounded-xl px-4 py-3.5 text-sm font-medium transition-all ${
                isActive
